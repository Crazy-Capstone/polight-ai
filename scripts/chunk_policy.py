...
# 조항 제목이 성격을 직접 선언하는 패턴.
#
# 제목을 본문보다 먼저 보는 이유는, 보장 조항이 면책 조항을 참조하는 일이 흔하기
# 때문이다. 본문만 보면 이런 것들이 전부 면책으로 뒤집힌다.
#
#   제4조의2(비급여 실손의료비 특별약관에서 보상하는 사항)
#     "① 제3조 및 제4조에도 불구하고 ... 기본형에서 보상하지 않습니다"
#                                              ^^^^^^^^^^^^^ 이 단어가 먼저 걸린다
#
#   ②회사가 보상하는 손해는 아래와 같습니다.
#     "... (단, 수하물 수령 이후 발생한 비용은 보상하지 않습니다.)"
#
# 실측에서 보장을 선언한 제목 56개 중 14개(25%)가 excluded로 잘못 분류됐다.
# 라벨이 뒤집히면 면책 짝짓기가 어긋나고, 프롬프트가 보장 조항을 "면책"으로
# 표시해 LLM이 "보상되지 않는다"고 답할 수 있다.
#
# 순서가 중요하다. "보상하지"를 먼저 봐야 "보상하지 않는 사항"이 보장으로 잡히지 않는다.
#
# 패턴에 공백을 넣지 않는다. 비교 전에 normalize_for_match가 공백을 모두 제거하므로
# "보상하지 않는"과 "보상하지않는"이 같은 문자열이 된다.
# 일반 조항 패턴. 담보가 아니라 계약 자체를 다루는 조항들이다.
#
# 이런 조항이 "보장"으로 라벨되면 프롬프트에 [보장]으로 표시돼, LLM이 담보 조항으로
# 오해할 수 있다. 백엔드 enum에 GENERAL이 있으므로 그것을 쓴다.
#
# 담보 관련 패턴보다 먼저 보지 않는다. "보험금의 지급사유"처럼 담보를 선언하는
# 제목에 "보험금"이 들어가므로, 일반 조항 패턴을 먼저 두면 담보가 GENERAL로 샌다.
GENERAL_TITLE_PATTERNS = (
    r"계약의(성립|무효|취소|해지|소멸|변경|부활)",
    r"알릴의무",
    r"약관의?교부",
    r"설명의무",
    r"주소의?변경",
...
TITLE_PATTERNS = (
    (r"보상하지(않|아니)", "excluded"),
    (r"지급하지(않|아니)", "excluded"),
    (r"면책", "excluded"),
    (r"보상하는(손해|사항|비용|내용)", "included"),
    (r"보상내용", "included"),
    (r"지급사유", "included"),
    (r"보험금의?청구", "procedure"),
    (r"지급절차", "procedure"),
    (r"청구서류", "procedure"),
    (r"용어의?(정의|해설|풀이)", "definition"),
)


def detect_coverage_type(title: str, text: str) -> str:
    # 1) 제목이 성격을 선언하면 그것을 따른다.
    #
    # 제목은 조항 작성자가 그 조항의 성격을 직접 밝힌 것이라 본문의 참조 문구보다
    # 신뢰도가 높다. 본문에 면책 참조가 섞여 있어도 제목이 "보상하는 사항"이면
    # 보장 조항이다.
    ...
    # 2) 담보 관련 선언이 없고 계약 일반 조항이면 general.
    #
    # 순서가 중요하다. 담보 패턴을 먼저 봐야 "보험료의 납입"과 "보험금의 지급사유"가
    # 섞이지 않는다. 앞은 일반 조항이고 뒤는 담보 선언이다.
    if any(re.search(p, normalized_title) for p in GENERAL_TITLE_PATTERNS):
        return "general"

    # 3) 제목으로 판단이 안 되면 본문을 본다 (기존 방식).
    #
