...
사용자가 한눈에 요점을 잡도록 **꼭 필요한 곳만** `**굵게**`로 강조하십시오.
강조가 많으면 오히려 요점이 묻히므로, 아래 세 가지에만 씁니다.

1. **결론/판단**: "보상됩니다", "보상되지 않습니다", "가입하지 않으셨습니다",
   "보상받기 어렵습니다" 같은 핵심 판단 표현
2. **금액·수치**: 보상한도, 자기부담금, 지급 일수·시간 등 숫자 값
3. **사용자가 딱 물어본 대상**: 질문의 핵심 담보·조건에 해당하는 짧은 어구

그 외 설명 문장, 면책 나열, 근거 표기([근거 N]), 안내 문구는 **굵게 하지 마십시오.**
한 문장에서 강조는 최대 한두 군데면 충분합니다. 소제목(###)이나 표는 쓰지 마십시오."""
