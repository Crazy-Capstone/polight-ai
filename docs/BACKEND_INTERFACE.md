...
| # | 필요한 것 | 왜 |
|---|---|---|
| 1 | **내부 API 키 값** | AI 서버가 콜백을 보낼 때, 백엔드가 AI를 호출할 때 양쪽이 같은 키를 쓴다. 지금 AI 쪽 `.env`가 비어 있어 인증이 꺼져 있다 |
| 2 | **백엔드 Base URL** | 콜백을 보낼 주소. 미설정이라 지금은 콜백을 건너뛰고 로그만 남긴다 |
| 3 | **파일 전달 방식** | S3 presigned URL인지, 백엔드가 파일을 내려주는 엔드포인트인지. 후자면 요청에 인증 헤더가 필요하다 |
...
### 3-2. 공유 약관 테이블 (나중에, 급하지 않음)

같은 보험 상품의 약관은 사용자가 달라도 내용이 같다. 한 번만 분석해 공유하면
비용이 크게 준다. 그런데 `policy_chunks`는 `user_id` / `analysis_result_id` /
`document_id`가 전부 NOT NULL이라 **주인 없는 공유 약관을 넣을 자리가 없다.**
...
**급하지 않은 이유**: 비용의 대부분인 파싱은 AI 쪽에서 파일 내용 해시로 캐시하도록
고쳐서, 같은 약관을 여러 사용자가 올려도 한 번만 파싱한다. 이 테이블은 임베딩 중복까지
없애는 것이라 이득이 상대적으로 작다. **여유 되실 때 해주시면 된다.**
...
| 여유 될 때 | 콜백의 `insurerName`/`productName`으로 약관 연결 | 챗봇이 약관 근거를 못 댐 |
| 여유 될 때 | RAG 요청에 `coverages` 실어주기 | 미가입·한도 답변이 부정확 |
| 여유 될 때 | `documentType` 실어주기 | 페이지 수로 자동 판별 (대개 맞음) |
| 나중 | `policy_terms` DDL | 임베딩 비용 중복 (기능은 정상) |

**위 4개를 안 해도 지금과 똑같이 동작한다.** 하나씩 채울 때마다 정확해질 뿐이다.
